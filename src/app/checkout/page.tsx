  ...
  return (
    <div className="min-h-screen bg-brand-charcoal">
      <div className="container-padded py-8 sm:py-12">
        <h1 className="font-serif text-2xl sm:text-3xl font-bold text-brand-white mb-2">Checkout</h1>
        <p className="text-brand-muted text-sm mb-8">Cash on Delivery — pay when your order arrives</p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-brand-charcoal rounded-xl p-5 border border-brand-mid/20 space-y-4">
                <h2 className="font-serif text-lg font-semibold text-brand-white">Contact Information</h2>
              ...
              {/* Delivery Address */}
              <div className="bg-brand-charcoal rounded-xl p-5 border border-brand-mid/20 space-y-4">
                <h2 className="font-serif text-lg font-semibold text-brand-white">Delivery Address</h2>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <select
                      {...register('district')}
                      className="bg-brand-dark border border-brand-mid rounded-md px-3 py-2.5 text-brand-white text-sm focus:outline-none focus:ring-2 focus:ring-brand-gold/60 focus:border-brand-gold"
                    >
                      {ALL_DISTRICTS.map((d) => (
                        <option key={d} value={d}>{d}</option>
                      ))}
                    </select>
                    {errors.district && <p className="text-red-400 text-xs">{errors.district.message}</p>}
                  </div>
                </div>
                <Input label="Postal Code" {...register('postalCode')} placeholder="Optional" />
                <div className="flex flex-col gap-1">
                  <label className="text-sm font-medium text-brand-light">Delivery Notes</label>
                  <textarea
              ...
              <div className="bg-brand-charcoal rounded-xl p-5 border border-brand-mid/20">
                <h2 className="font-serif text-lg font-semibold text-brand-white mb-3">Payment Method</h2>
                <div className="flex items-center gap-3 bg-brand-gold/5 border border-brand-gold/30 rounded-lg p-4">
                  <div className="w-4 h-4 rounded-full border-2 border-brand-gold bg-brand-gold shrink-0" />
                  <div>
                    <p className="text-brand-white font-semibold text-sm">Cash on Delivery (COD)</p>
                    <p className="text-brand-muted text-xs">Pay with cash when your order is delivered</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="lg:col-span-1">
              <div className="bg-brand-charcoal rounded-xl p-5 border border-brand-mid/20 sticky top-20">
                <h2 className="font-serif text-lg font-bold text-brand-white mb-4">Order Summary</h2>
                  ...
                  {items.map((item) => (
                    <div key={item.sku} className="flex gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-brand-white text-xs truncate">{item.title}</p>
                        <p className="text-brand-muted text-xs">{item.size}ml × {item.quantity}</p>
                        <p className="text-brand-gold text-xs font-semibold">{formatPrice((item.salePrice ?? item.price) * item.quantity)}</p>
                      </div>
                    </div>
                  ))}
                </div>
                <div className="border-t border-brand-mid/30 pt-4 space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-brand-muted">Subtotal</span>
                    <span className="text-brand-white">{formatPrice(subtotal)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-brand-muted">Delivery ({watchedDistrict || 'Colombo'})</span>
                    <span className="text-brand-white">{formatPrice(deliveryFee)}</span>
                  </div>
                  <div className="flex justify-between font-bold pt-2 border-t border-brand-mid/30">
